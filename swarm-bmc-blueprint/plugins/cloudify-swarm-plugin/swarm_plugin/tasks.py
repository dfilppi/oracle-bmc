...
@operation
def manager_in_host(**kwargs):
  ctx.logger.info("getting ip props={},{}".format(kwargs['private_ip'],kwargs['public_ip']))
  ctx.source.instance.runtime_properties['private_ip']=kwargs['private_ip']
  ctx.source.instance.runtime_properties['public_ip']=kwargs['public_ip']
...
def add_microservice(**kwargs):
  ctx.logger.info("in add_microservice")
  ctx.source.instance.runtime_properties['ip']=ctx.target.instance.runtime_properties['public_ip']
...
def rm_service(**kwargs):
  ...
  resp = requests.delete('http://{}:{}/services/{}'.format(ctx.node.properties['ip'],ctx.instance.runtime_properties['port'],id))
